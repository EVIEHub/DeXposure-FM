...
PREPROCESSING_CONTRACT = {
    "contract_version": 2,
    "node_id_policy": "stringify_non_null_then_strip; count_invalid_and_skip_null_or_blank",
    "duplicate_node_policy": "count_invalid_and_keep_first_record",
    "metadata_id_policy": "stringify_non_null_then_strip; reject_null_or_blank_and_duplicates",
    "missing_node_size_policy": "use_zero_and_mark_log_size_missing",
    "edge_policy": {
        "endpoint_policy": "count_null_or_blank_or_unknown_as_invalid",
        "weight_policy": "count_missing; reject_non_finite_or_negative",
        "parallel_edges": "sum_by_directed_source_target",
        "output_order": "lexicographic_source_target",
        "zero_weight": "exclude_after_validation",
    },
    "feature_order": list(FEATURE_ORDER),
    "historical_feature": "log1p(current_positive)-log1p(previous_positive),_else_zero",
    "degree_features": "count_coalesced_directed_edges_then_divide_by_per_snapshot_max_or_one",
}
...
def preprocessing_contract_hash() -> str:
    return hashlib.sha256(_canonical_json(PREPROCESSING_CONTRACT).encode()).hexdigest()


def normalize_node_id(value: Any, *, field: str = "node id") -> str:
    """Canonicalize an identifier or fail closed at the raw-data boundary."""
    if value is None:
...
def load_network_data(path: str) -> Dict:
    ...
    if not isinstance(data, dict):
        raise ValueError(f"Network data must be a date map: {path}")
    for date, snapshot in data.items():
        if not isinstance(date, str):
            raise ValueError(f"Snapshot date keys must be strings: {path}")
        try:
            datetime.strptime(date, "%Y-%m-%d")
        except ValueError as exc:
            raise ValueError(f"Invalid snapshot date {date!r} in {path}") from exc
        if not isinstance(snapshot, dict):
            raise ValueError(f"Snapshot {date} must be an object: {path}")
    return data
...
def load_metadata(path: str) -> Tuple[Dict, List[str], Dict]:
    ...
    missing_columns = required - set(meta_df.columns)
    if missing_columns:
        raise ValueError(f"Metadata missing columns: {sorted(missing_columns)}")
    if meta_df["id"].isna().any():
        raise ValueError(f"Metadata contains missing ids: {path}")
    meta_df["id"] = meta_df["id"].astype("string").str.strip()
    if (meta_df["id"] == "").any():
        raise ValueError(f"Metadata contains empty ids: {path}")
    if meta_df["id"].duplicated().any():
        duplicates = sorted(
            meta_df.loc[meta_df["id"].duplicated(), "id"].unique()
        )
        raise ValueError(f"Duplicate metadata ids: {duplicates[:10]}")
    meta_df["category"] = (
        meta_df["category"]
        .fillna("Unknown")
        .astype("string")
...
def build_feature_schema(meta_path: str, category_list: List[str]) -> Dict[str, Any]:
    """Freeze the exact inference contract used by a trained checkpoint."""
    metadata_bytes = Path(meta_path).read_bytes()
    return {
        "schema_version": 2,
        "metadata_sha256": hashlib.sha256(metadata_bytes).hexdigest(),
        "feature_order": list(FEATURE_ORDER),
        "categories": list(category_list),
        "edge_weight_space": "log1p",
        "node_target_space": "log1p_delta",
...
def write_feature_schema(
    ...
    schema = build_feature_schema(meta_path, category_list)
    output_dir.mkdir(parents=True, exist_ok=True)
    schema_path = output_dir / "feature_schema.json"
    schema_path.write_text(json.dumps(schema, indent=2) + "\n")
    log_info(f"  Feature schema saved to: {schema_path}")
    return schema


def sha256_file(path: str | Path) -> str:
    """Hash a file in bounded chunks for checkpoint provenance."""
...
def _is_sha256_hex(value: Any) -> bool:
    """Return whether ``value`` is a canonical-length SHA-256 digest."""
    return (
...
def _preflight_source_identity() -> Dict[str, Any]:
    """Capture the host-bound source digest passed through ``cloud/train.sh``.

    The worker intentionally has no ``.git`` directory.  A missing digest is
    therefore explicit diagnostic provenance rather than an assertion that a
    git commit identifies the exact synced source tree.
    """
    raw_digest = _os.environ.get("PREFLIGHT_SOURCE_SHA256", "").strip()
    if raw_digest:
        if not _is_sha256_hex(raw_digest):
            raise ValueError("PREFLIGHT_SOURCE_SHA256 must be a 64-hex SHA-256 digest")
        return {
            "source_digest_sha256": raw_digest.lower(),
            "preflight_source_sha256": raw_digest.lower(),
            "source_digest_kind": "preflight_manifest",
            "source_digest_diagnostic": False,
        }
    return {
        "source_digest_sha256": None,
        "preflight_source_sha256": None,
        "source_digest_kind": "diagnostic_git_or_file_only",
        "source_digest_diagnostic": True,
    }


def _git_identity(repo_root: Path) -> Dict[str, Any]:
    """Capture source revision and a deterministic dirty-tree identity."""
    ...
    commit = (run("rev-parse", "HEAD") or "").strip() or "unknown"
    status = run("status", "--porcelain=v1") or ""
    diff = run("diff", "--binary", "HEAD") or ""
    ...
    return {
        ...
        "dirty_patch_sha256": hashlib.sha256(patch_payload).hexdigest(),
    }
...
def build_checkpoint_provenance(
    *,
    ...
    best_validation_metric_name: str,
    checkpoint_path: Optional[str] = None,
) -> Dict[str, Any]:
    """Build the immutable identity attached to every newly saved checkpoint."""
    if not math.isfinite(float(best_validation_metric)):
        raise ValueError("best_validation_metric must be finite")
    source_identity = {
        **_git_identity(GRAPHPFN_ROOT.parent.parent),
    ...
    return {
        ...
        "horizon": int(horizon),
        "preprocessing_contract_hash": feature_schema.get(
            "preprocessing_contract_hash", preprocessing_contract_hash()
        ),
        "best_validation_metric": float(best_validation_metric),
        "best_validation_metric_name": str(best_validation_metric_name),
    }
...
def node_features(
    ...
    if not math.isfinite(size):
        raise ValueError(f"Node {node_id!r} has non-finite size: {size}")
    comp = node.get("composition", {}) or {}
    if not isinstance(comp, dict):
        raise ValueError(f"Node {node_id!r} composition must be an object")
    ...
    if size > 0 and comp:
        ...
        if not np.isfinite(values).all():
            raise ValueError(f"Node {node_id!r} has non-finite composition values")
        values = np.maximum(values, 0.0)
        total = values.sum() + EPS
        shares = values / total
        max_share = float(shares.max())
        entropy = max(0.0, float(-(shares * np.log(shares + EPS)).sum()))
    else:
        max_share = 0.0
        entropy = 0.0
...
def build_snapshot(
    ...
    if not isinstance(nodes, list) or not isinstance(links, list):
        raise ValueError(f"Snapshot {date} requires list-valued nodes and links")
    ...
    for link in links:
        if not isinstance(link, dict):
            raise ValueError(f"Snapshot {date} contains a non-object link record")
        ...
        if source is None or target is None:
            ...
            continue
        try:
            source = normalize_node_id(source, field="edge source")
            target = normalize_node_id(target, field="edge target")
        except ValueError:
            invalid_edge_records += 1
            continue
        ...
        if not math.isfinite(weight) or weight < 0.0:
            raise ValueError(
                f"Invalid edge weight {weight} for {source!r}->{target!r} in snapshot {date}"
            )
        if weight > 0.0:
            pair = (source, target)
    ...
    for source, target in sorted(edge_weights):
        ...
        weights.append(edge_weights[(source, target)])
    ...
    return {
        ...
        "source_node_records_total": len(nodes),
        "source_node_records_invalid": invalid_node_records,
        "source_edge_records_total": len(links),
        "source_edge_records_invalid": invalid_edge_records,
    }
...
def enrich_snapshots_with_historical_features(snapshots: List[Dict]) -> List[Dict]:
    ...
    for snap in snapshots:
        size_map = {}
        for nid, size in zip(snap["node_ids"], snap["sizes"]):
            size_map.setdefault(nid, size)
...
def expanding_window_split(
    ...
    if len(all_dates) != len(set(all_dates)):
        raise ValueError("all_dates must be unique")
    if min_train_weeks <= 0 or val_weeks <= 0 or test_weeks <= 0 or step_weeks <= 0:
        raise ValueError("split window sizes must be positive")
    if step_weeks < test_weeks:
        raise ValueError("step_weeks must be at least test_weeks to avoid overlap")
    try:
        datetime.fromisoformat(holdout_start)
        for date in all_dates:
            datetime.fromisoformat(date)
    except ValueError as exc:
        raise ValueError(f"split dates must use YYYY-MM-DD format: {exc}") from exc
    ...
    if len(pre_holdout) <= val_weeks:
        raise ValueError("pre-holdout data must exceed val_weeks")
...
def build_scenario_label_map(
    ...
    for event in SHOCK_EVENTS:
        if event.event_date not in label_map:
            continue
        event_idx = dates.index(event.event_date)
...
def expected_graphpfn_adapter_keys(num_layers: int = 12) -> set[str]:
    """Return the exact adapter keys shipped by graphpfn-v1.ckpt."""
    keys: set[str] = set()
    for layer in range(num_layers):
        prefix = f"tfm.transformer_encoder.layers.{layer}"
        keys.update(
            {
                f"{prefix}.conv.norm.weight",
                f"{prefix}.conv.norm.bias",
                f"{prefix}.conv.base.attn_qkv_linear.weight",
                f"{prefix}.conv.base.attn_qkv_linear.bias",
                f"{prefix}.conv.base.output_linear.weight",
                f"{prefix}.conv.base.output_linear.bias",
                f"{prefix}.mlp.norm.weight",
                f"{prefix}.mlp.norm.bias",
                f"{prefix}.mlp.base.layers.0.weight",
                f"{prefix}.mlp.base.layers.0.bias",
                f"{prefix}.mlp.base.layers.2.weight",
                f"{prefix}.mlp.base.layers.2.bias",
            }
        )
    return keys


def validate_graphpfn_checkpoint_keys(
    model: Any, checkpoint_state: Dict[str, Any]
) -> None:
    """Fail closed on a missing adapter key or an unknown checkpoint key."""
    expected = expected_graphpfn_adapter_keys()
    actual = set(checkpoint_state)
    model_keys = set(model.state_dict())
    missing_expected = sorted(expected - actual)
    unknown = sorted(actual - model_keys)
    if missing_expected or unknown:
        raise ValueError(
            "GraphPFN checkpoint key contract failed: "
            f"missing_expected={missing_expected[:8]}, "
            f"unknown={unknown[:8]}"
        )
...
if GRAPHPFN_AVAILABLE:
    ...
    def load_graphpfn_encoder(checkpoint_path: str, device: torch.device) -> GraphPFN:
        """Load the adapter checkpoint with an explicit key contract."""
        model = GraphPFN(edge_head=False)
        checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
        if not isinstance(checkpoint, dict) or not isinstance(checkpoint.get("model"), dict):
            raise ValueError(f"GraphPFN checkpoint has no model state: {checkpoint_path}")
        checkpoint_state = checkpoint["model"]
        validate_graphpfn_checkpoint_keys(model, checkpoint_state)
        missing, unexpected = model.load_state_dict(checkpoint_state, strict=False)
        if unexpected:
            raise ValueError(
                f"GraphPFN checkpoint has unexpected loaded keys: {unexpected[:8]}"
            )
        expected = expected_graphpfn_adapter_keys()
        if expected - (set(model.state_dict()) - set(missing)):
            raise ValueError("GraphPFN checkpoint adapter keys were not loaded")
        model.to(device)
        return model
...
def train_graphpfn_epoch(
    ...
    # Store embeddings only when temporal smoothness is active. Keeping two
    # full epochs of embeddings on the GPU when its loss weight is zero wastes
    # memory without changing the objective.
    track_temporal_embeddings = config.smooth_loss_weight > 0.0
    ...
    }, current_embeddings if track_temporal_embeddings else None
...
def run_graphpfn_experiment(
    ...
    feature_schema = build_feature_schema(config.meta_path, category_list)
    ...
    for horizon in config.forecast_horizons:
        ...
        if not train_pairs or not val_pairs or not test_pairs:
            raise ValueError(
                f"h={horizon} requires non-empty train, validation, and test pairs"
            )
        ...
        completed_epochs = 0
        split_payload = {
            "train": [snap["date"] for snap in train_snaps],
            "val": [snap["date"] for snap in val_snaps or []],
            "test": [snap["date"] for snap in test_snaps or []],
        }
        checkpoint_provenance_template = None

        def checkpoint_provenance_for(
            epochs_completed: int, validation_metric: float
        ) -> Dict[str, Any]:
            nonlocal checkpoint_provenance_template
            if checkpoint_provenance_template is None:
                checkpoint_provenance_template = build_checkpoint_provenance(
                    data_path=config.data_path,
                    metadata_path=config.meta_path,
                    split=split_payload,
                    seed=config.seed,
            ...
            provenance = copy.deepcopy(checkpoint_provenance_template)
            provenance["completed_epochs"] = int(epochs_completed)
            provenance["best_validation_metric"] = float(validation_metric)
            return provenance
        ...
        for epoch in range(config.epochs):
            ...
            if any(not math.isfinite(float(value)) for value in losses.values()):
                raise RuntimeError(f"h={horizon} produced non-finite training loss")
            ...
            if val_pairs and (epoch + 1) % config.val_eval_every == 0:
                ...
                if not math.isfinite(val_metric):
                    raise RuntimeError(
                        f"h={horizon} validation {metric_name} is undefined"
                    )
                ...
                if val_metric > best_metric:
                    ...
                    if model_output_dir:
                        partial_path = (
                            model_output_dir
                            / f"in_progress_best_model_h{horizon}.partial.pt"
                        )
                        partial_tmp = partial_path.with_suffix(".pt.tmp")
                        partial_provenance = checkpoint_provenance_for(
                            epoch + 1, best_metric
                        )
                        torch.save(
                            {
        ...
        if best_model_state is not None:
            ...
            try:
                if model_output_dir:
                    ...
                    provenance = checkpoint_provenance_for(
                        completed_epochs, best_metric
                    )
                    torch.save(
                        {
        ...
        if not all(
            math.isfinite(float(test_metrics[group][metric]))
            for group, metrics in required_test_metrics.items()
            for metric in metrics
        ):
            raise RuntimeError(f"h={horizon} produced a non-finite required test metric")
        ...
        if model_output_dir:
            ...
            with open(intermediate_path, "w") as f:
                json.dump(
                    json_safe(intermediate_result), f, indent=2, allow_nan=False
                )
...
def run_roland_experiment(
    ...
    for horizon in config.forecast_horizons:
        ...
        if not train_pairs or not val_pairs or not test_pairs:
            raise ValueError(
                f"ROLAND h={horizon} requires non-empty train, validation, and test pairs"
            )
        ...
        for epoch in range(config.epochs):
            ...
            if any(not math.isfinite(float(value)) for value in losses.values()):
                raise RuntimeError(f"ROLAND h={horizon} produced non-finite training loss")
            weighted_parts = {
            ...
            if val_pairs and (epoch + 1) % config.val_eval_every == 0:
                ...
                if not math.isfinite(val_metric):
                    raise RuntimeError(
                        f"ROLAND h={horizon} validation {metric_name} is undefined"
                    )
                if val_metric > best_metric:
                    best_metric = val_metric
                    patience_counter = 0
        ...
        test_metrics = evaluate_predictions(test_preds)
        if not all(
            math.isfinite(test_metrics["exist"][metric])
            for metric in ("auprc", "auroc")
        ):
            raise RuntimeError(
                f"ROLAND h={horizon} test existence metrics are undefined"
            )
        results[f"h{horizon}"] = test_metrics
...
def main():
    ...
    parser.add_argument(
        ...
        help="Experiment mode: all, frozen, dexposure-fm (alias: deposure-fm), roland, persistence (naive baseline), stats",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Test window size per fold in weeks (default: 16)",
    )
    parser.add_argument(
        "--step-weeks",
        type=int,
        default=16,
        help="Step size for non-overlapping expanding-window tests (default: 16)",
    )
    parser.add_argument(
        "--rolling",
        action="store_true",
    ...
    try:
        forecast_horizons = [int(h.strip()) for h in args.horizons.split(",")]
    except ValueError as exc:
        parser.error(f"--horizons must be comma-separated integers: {exc}")
    if (
        not forecast_horizons
        or len(forecast_horizons) != len(set(forecast_horizons))
        or any(horizon not in {1, 4, 8, 12} for horizon in forecast_horizons)
    ):
    ...
    if args.val_eval_every <= 0:
        parser.error("--val-eval-every must be positive")
    if args.min_train_weeks <= 0:
        parser.error("--min-train-weeks must be positive")
    if args.val_weeks <= max(forecast_horizons):
        parser.error("--val-weeks must exceed the largest forecast horizon")
    if args.rolling and args.test_weeks <= max(forecast_horizons):
        parser.error("--test-weeks must exceed the largest horizon in rolling mode")
    if args.test_weeks <= 0 or args.step_weeks <= 0:
        parser.error("--test-weeks and --step-weeks must be positive")
    if args.rolling and args.step_weeks < args.test_weeks:
        parser.error("--step-weeks must be at least --test-weeks to avoid overlapping tests")
    if not math.isfinite(args.gradient_clip_norm) or args.gradient_clip_norm < 0:
        parser.error("--gradient-clip-norm must be finite and non-negative")
    try:
        datetime.fromisoformat(args.holdout_start)
    except ValueError as exc:
        parser.error(f"--holdout-start must be YYYY-MM-DD: {exc}")
    ...
    config.val_eval_every = args.val_eval_every
    config.seed = args.seed
    config.forecast_horizons = forecast_horizons
...
def _run_experiments(args, config: ExperimentConfig, output_dir: Path):
    ...
    log_info("\nLoading data...")
    meta_category, category_list, category_to_idx = load_metadata(config.meta_path)
    write_feature_schema(output_dir, config.meta_path, category_list)
    network_data = load_network_data(config.data_path)
    all_dates = sorted(network_data.keys())
    ...
    with open(output_dir / "data_quality.json", "w") as f:
        json.dump(json_safe(data_quality), f, indent=2, allow_nan=False)
    ...
    def prepare_mode_output(mode_name: str) -> Path:
        """Create a per-mode directory without replacing the root result manager."""
        if args.mode != "all":
            return output_dir
        ...
        with open(mode_output_dir / "config.json", "w") as f:
            json.dump(
                {**config_payload, "mode": mode_name, "output_dir": str(mode_output_dir)},
                f,
                indent=2,
            )
    ...
    if args.rolling and run_models:
        ...
        try:
            ...
            task_failures.append({"task": "rolling_evaluation", "error": str(e)})
    ...
    if args.mode in ["all", "frozen"]:
        ...
        try:
            ...
            task_failures.append({"task": "graphpfn_frozen", "error": str(e)})
    ...
    if args.mode in ["all", "dexposure-fm", "deposure-fm"]:
        ...
        try:
            ...
            task_failures.append({"task": "dexposure_fm", "error": str(e)})
    ...
    if args.mode in ["all", "roland"]:
        ...
        try:
            ...
            task_failures.append({"task": "roland", "error": str(e)})
    ...
    if args.mode in ["all", "persistence"]:
        ...
        try:
            ...
            for horizon in config.forecast_horizons:
                ...
                if not test_pairs_h:
                    raise ValueError(f"Persistence h={horizon} has no test pairs")
                metrics = evaluate_persistence_baseline(test_pairs_h)
                if not all(
                    math.isfinite(metrics["exist"][metric])
                    for metric in ("auprc", "auroc")
                ):
                    raise RuntimeError(
                        f"Persistence h={horizon} existence metrics are undefined"
                    )
            ...
            task_failures.append({"task": "persistence", "error": str(e)})
    ...
    if args.mode in ["all", "stats"]:
        ...
        try:
            ...
            task_failures.append({"task": "network_stats", "error": str(e)})

    all_results["status"] = "failed" if task_failures else "complete"
    all_results["task_failures"] = task_failures
    ...
    with open(all_results_path, "w") as f:
        json.dump(
            json_safe(all_results), f, indent=2, default=str, allow_nan=False
        )
    ...
    if task_failures:
        failed_names = ", ".join(item["task"] for item in task_failures)
        raise RuntimeError(f"Experiment tasks failed: {failed_names}")
