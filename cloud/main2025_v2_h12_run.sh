#!/usr/bin/env bash
# Reconstruct one DeXposure-FM v2 checkpoint for the 12-week horizon.
set -euo pipefail
...
PREFLIGHT_CMD="bash cloud/main2025_v2_h12_run.sh"
...
TRAIN_HORIZONS=12
TRAIN_ROOT="checkpoints/main2025_v2_h12_train"
TRAIN_OUTPUT="$TRAIN_ROOT/finetuned"
RELEASE_ROOT="checkpoints/main2025_v2_h12"

echo "=== DeXposure-FM v2 reconstruction (h=12, holdout=2025-01-01, val=24, epochs=20, seed=42) ==="
...
src="$TRAIN_OUTPUT/best_model_h12.pt"
...
cp "$src" "$RELEASE_ROOT/dexposure-fm-h12.pt"
cp "$schema_src" "$RELEASE_ROOT/feature_schema.json"
shasum -a 256 "$RELEASE_ROOT/dexposure-fm-h12.pt" \
  | tee "$RELEASE_ROOT/SHA256SUMS"

echo "MAIN 2025 V2 H12 CHECKPOINT RUN DONE"
