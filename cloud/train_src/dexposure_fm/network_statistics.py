...
def sector_connectivity_matrix(
    ...
    if len(edge_weights) != edge_index.shape[1]:
        raise ValueError("edge_weights length must equal edge count")
    if len(node_sectors) == 0 and edge_index.shape[1] > 0:
        raise ValueError("node_sectors cannot be empty when edges are present")
    ...
    for e in range(edge_index.shape[1]):
        ...
        if src < 0 or dst < 0 or src >= len(node_sectors) or dst >= len(node_sectors):
            raise ValueError("edge index is outside node_sectors")
        src_sector = node_sectors[src]
        dst_sector = node_sectors[dst]
...
def compute_all_network_statistics(
    ...
    if num_edges:
        ...
        if np.min(edge_index) < 0 or np.max(edge_index) >= num_nodes:
            raise ValueError("edge_index contains an out-of-range node index")
    if edge_weights is not None and len(edge_weights) != num_edges:
        raise ValueError("edge_weights length must equal edge count")
    if node_sizes is not None and len(node_sizes) != num_nodes:
        raise ValueError("node_sizes length must equal num_nodes")
    if node_sectors is not None and len(node_sectors) != num_nodes:
        raise ValueError("node_sectors length must equal num_nodes")
